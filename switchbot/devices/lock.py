...
from ..api_config import SWITCHBOT_ACCOUNT_API_BASE_URL, SWITCHBOT_APP_API_BASE_URL
from ..const import (
    LockStatus,
...
class SwitchbotLock(SwitchbotDevice):
    ...
    @staticmethod
    def post(
        log_name: str, url: str, body: dict, access_token: str | None = None
    ) -> dict:
        try:
            ...
            raise SwitchbotAccountConnectionError(
                f"Failed to retrieve {log_name} from SwitchBot Account: {err}"
            ) from err
        if response.status_code > 299:
            raise SwitchbotAuthenticationError(
                f"Unexpected status code returned by SwitchBot Account API: {response.status_code}"
            )
        resp = response.json()
        if resp["statusCode"] != 100:
            raise SwitchbotAuthenticationError(
                f"Unexpected status code returned by SwitchBot API: {resp['statusCode']}"
            )
        return resp
    ...
    def retrieve_encryption_key(device_mac: str, username: str, password: str):
        ...
        auth_response = SwitchbotLock.post(
            log_name="login credentials",
            url=SWITCHBOT_ACCOUNT_API_BASE_URL + "/account/api/v1/user/login",
            body={
        ...
        user_response = SwitchbotLock.post(
            log_name="user info",
            url=SWITCHBOT_ACCOUNT_API_BASE_URL + "/account/api/v1/user/userinfo",
            body={},
            access_token=access_token,
        )
        user_region = (
            user_response["body"]["botRegion"]
            if user_response["body"]["botRegion"]
            else "us"
        )

        key_response = SwitchbotLock.post(
            log_name="encryption key",
            url=SWITCHBOT_APP_API_BASE_URL.format(user_region) + "/keys/v1/communicate",
            body={
        ...
        return {
            "key_id": key_response["body"]["communicationKey"]["keyId"],
            "encryption_key": key_response["body"]["communicationKey"]["key"],
        }
